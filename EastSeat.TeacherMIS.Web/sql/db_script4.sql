...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'Teacher') AND [c].[name] = N'CurrentPosition');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [Teacher] DROP CONSTRAINT [' + @var3 + '];');
ALTER TABLE [Teacher] DROP COLUMN [CurrentPosition];

GO

ALTER TABLE [Teacher] ADD [GradeId] uniqueidentifier;

GO

ALTER TABLE [Teacher] ADD [PositionId] uniqueidentifier;
...
CREATE INDEX [IX_Teacher_GradeId] ON [Teacher] ([GradeId]);

GO

CREATE INDEX [IX_Teacher_PositionId] ON [Teacher] ([PositionId]);

GO

ALTER TABLE [Teacher] ADD CONSTRAINT [FK_Teacher_Grade_GradeId] FOREIGN KEY ([GradeId]) REFERENCES [Grade] ([GradeId]) ON DELETE NO ACTION;

GO

ALTER TABLE [Teacher] ADD CONSTRAINT [FK_Teacher_Position_PositionId] FOREIGN KEY ([PositionId]) REFERENCES [Position] ([PositionId]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20170906071737_TeacherGradeAndPosition', N'1.1.2');
